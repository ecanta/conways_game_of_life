...
- **Strumenti di disegno**: pennello, gomma, selezione rettangolare (con interpolazione del tratto, quindi disegno fluido anche a mouse veloce).
...
- **Multi-file** (default): usa `resource.h` e il file `.rc` associato, già presenti nel repository, che definiscono l'icona del programma (`IDI_APPICON`, usata come icona della finestra) e le icone dei pulsanti pennello/gomma/selezione nei rispettivi stati on/off (`IDI_BRUSH`, `IDI_ERASER`, `IDI_BRUSH_ON`, `IDI_ERASER_ON`, `IDI_SELECT`, `IDI_SELECT_ON`).
